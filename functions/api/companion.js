...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (contentLength > 50000) {
      return json({ error: "The conversation is too large. Please begin a new walk." }, 413, origin);
    }
    ...
    if (!messages.length || messages.length > 24) {
      return json({ error: "Please begin a new walk." }, 400, origin);
    }
    ...
    if (typeof body.system === "string" && body.system.length <= 12000) {
    ...
    console.error("Companion function error", err);
    return json({ error: "Something interrupted the walk. Please try again." }, 500, origin);
  }
}
